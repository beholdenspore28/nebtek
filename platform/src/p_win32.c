...
void p_start(void) {
  ...
  pfd.nSize = sizeof(PIXELFORMATDESCRIPTOR); // Set the size of the PFD to the
                                             // size of the class
  pfd.dwFlags = PFD_DOUBLEBUFFER | PFD_SUPPORT_OPENGL |
                PFD_DRAW_TO_WINDOW; // Enable double buffering, opengl support
                                    // and drawing to a window
  pfd.iPixelType = PFD_TYPE_RGBA;   // Set our application to use RGBA pixels
  pfd.cColorBits =
      32; // Give us 32 bits of color information (the higher, the more colors)
  pfd.cDepthBits = 32; // Give us 32 bits of depth information (the higher, the
                       // more depth levels)
  pfd.iLayerType = PFD_MAIN_PLANE; // Set the layer of the PFD
  int format = ChoosePixelFormat(internal_state.hdc, &pfd);
  if (format == 0 ||
      SetPixelFormat(internal_state.hdc, format, &pfd) == FALSE) {
  ...
  if (NULL == (opengl_context = wglCreateContextAttribsARB(internal_state.hdc,
                                                           NULL, attributes))) {
  ...
  wglMakeCurrent(internal_state.hdc,
                 opengl_context); // Make our OpenGL 3.2 context current
...
void p_process_events(void) {
  while (
      PeekMessage(&internal_state.msg, internal_state.hwnd, 0, 0, PM_REMOVE)) {
    TranslateMessage(&internal_state.msg);
    DispatchMessage(&internal_state.msg);

    if (internal_state.msg.message == WM_QUIT ||
        (internal_state.msg.message == WM_KEYDOWN &&
         internal_state.msg.wParam == VK_ESCAPE))
