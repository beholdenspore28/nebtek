...
_Bool p_is_running(void) {
  return internal_state.is_running;
}
...
void p_start(void) {
    ...
    internal_state.visual_info = glXChooseVisual(internal_state.display, screen, visual_attributes);

    internal_state.colormap = XCreateColormap(internal_state.display, root, internal_state.visual_info->visual, AllocNone);
    ...
    internal_state.window =
        XCreateWindow(internal_state.display, root, 0, 0, window_width, window_height, 0,
                      internal_state.visual_info->depth, InputOutput, internal_state.visual_info->visual,
                      CWColormap | CWEventMask, &attributes);

    XMapWindow(internal_state.display, internal_state.window);
    XStoreName(internal_state.display, internal_state.window, "[glad] GLX with X11");
    ...
    internal_state.context = glXCreateContext(internal_state.display, internal_state.visual_info, NULL, GL_TRUE);
    glXMakeCurrent(internal_state.display, internal_state.window, internal_state.context);
